...
exports.analyzeFolder = async (req, res) => {
    try {
        ...
        if (courses.length === 0) return res.status(200).json({ motivationalMessage: "There's no course in this folder. Create a new course to start!" });
        
        const courseIds = courses.map(c => c._id);
        const grades = await Grade.find({ courseId: { $in: courseIds } });
        if (grades.length === 0) return res.status(200).json({ motivationalMessage: "You don't have any grades in this folder. Please add grades to receive analysis from AI!" });
        ...
        try {
            ...
            return res.status(200).json({ 
                error: true,
                message: "Unsupported format from AI, this is the raw data:", 
        ...
        res.status(500).json({ message: "Analysis from server failed", error: error.message });
    }
};